...
p2 = plot(load_factors .* 100, timings, yscale = :log10, xlabel = "Total Load (% of peak load)", ylabel = "Execution time [s]", legend = false)
plot(p1, p2, layout = (2, 1))
plot!(size=(600,800))
savefig("pscopf_analysis.png")

## Validation of base case
param2 = deepcopy(param);
param2.Pd .= 0.5.*param.Pd;

model_base = create_base_model(set, param2);
model_base[:param] = param2;
model_base[:set] = set;

optimize!(model_base);

cost_base = objective_value(model_base);
base_Pg = deepcopy(value.(model_base[:Pg]));
base_Pk = deepcopy(value.(model_base[:Pk]));
base_Pw = deepcopy(value.(model_base[:Pw]));
bases_theta = deepcopy(value.(model_base[:theta]));

#check if OPF feasible for all single generator outages
unsafe_contingencies = check_contingencies!(model_base, Vector{Int64}());
@assert length(unsafe_contingencies) == 0

unsafe_contingencies = pscopf!(model_base);
@assert length(unsafe_contingencies) == 0;
@assert norm(base_Pg - value.(model_base[:Pg])) < 1e-10
@assert norm(base_Pw - value.(model_base[:Pw])) < 1e-10
@assert norm(base_Pk - value.(model_base[:Pk])) < 1e-10
@assert norm(bases_theta - value.(model_base[:theta])) < 1e-10
